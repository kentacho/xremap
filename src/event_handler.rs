...
use crate::config::modmap_action::{ModmapAction, MultiPurposeKey, PressReleaseKey, StickyKey};
...
impl EventHandler {
    ...
    fn on_key_event(&mut self, event: &KeyEvent, config: &Config) -> Result<(), Box<dyn Error>> {
        ...
        let mut key_values = if let Some(key_action) = self.find_modmap(config, &key) {
            if let ModmapAction::StickyKey(sticky_key_) = &key_action {
    ...
    fn dispatch_keys(
        ...
        let keys = match key_action {
            ...
            ModmapAction::StickyKey(sticky_key) => {
